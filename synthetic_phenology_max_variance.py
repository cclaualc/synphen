...
######### PROGRAM SYNTHETIC PHENOLOGY MAX VARIANCE ###########
...
# In this case we maximize the variance associated to the middle dates
...
########### VARIANCE of the MIDDLE DATES ##################
## Calculates the negative variance of the middle dates 
def variance(x, n_total, periods):
      ...
      mean = 0
      for i in range(n_total):
            mean = mean + tm[i]/float(n_total)
      #Calculating the variance
      suma = 0
      for i in range(n_total):
            suma = suma + (tm[i] - mean)*(tm[i] - mean)
      #We add a negative sign in order to ensure that the variance is maximized  
      var = -suma/float(n_total)
...
for i in range(Nsample):
      ...
      res = minimize(variance, t0, args = (n_total,periods), method='trust-constr', bounds=season_bounds, constraints=(LinearConstraint(A, lb, ub, keep_feasible=False)), tol=0.1,callback=None, options={'disp':False})
...
res=differential_evolution(variance, bounds=season_bounds, args=(n_total, periods), strategy='best1exp', maxiter=1000000000000, tol=0.1,disp=False, polish=True, init=init_pop,constraints=(LinearConstraint(A, lb, ub,keep_feasible=False)))
...
#Opening files
doc_periods = open ("maxvar_periods"+idoc+".dat","w+")
doc_ti = open ("maxvar_ti"+idoc+".dat","w+")
...
plt.draw()
plt.show()
fig.savefig('maxvar_periods'+idoc+'.png')
plt.clf()
